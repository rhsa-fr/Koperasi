...
export default function AnggotaPage() {
  ...
  // ✅ Cek izin dinamis — mendukung RBAC
  const { user, can } = useAuth()
  const canCreate = can('anggota', 'create')
  const canUpdate = can('anggota', 'update')
  const canDelete = can('anggota', 'delete')
  const canEdit = canUpdate || canDelete // Untuk kolom aksi
            ...
            {loading ? (
              ...
              data.map((a, idx) => (
                ...
                >
                  ...
                  {/* ✅ Tombol edit/hapus berdasarkan izin dinamis */}
                  {canEdit && (
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        {canUpdate && (
